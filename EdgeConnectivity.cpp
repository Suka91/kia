...
void createDirectedGraph (vector<vector<int>> &G, vector<UndirectedEdge> &UndirectedEdges, vector<vector<int>> &N, vector<Edge> &Edges) {
    ...
    for (UndirectedEdge unEdge: UndirectedEdges) {
        ...
        insertDirectedEdge(N, Edges, u, a, 1);
        insertDirectedEdge(N, Edges, v, a, 1);

        insertDirectedEdge(N, Edges, a, b, 1);

        insertDirectedEdge(N, Edges, b, u, 1);
        insertDirectedEdge(N, Edges, b, v, 1);
    }
}
...
int main () {
    ...
    for (int s = 0; s < n - 1; s++) {
        for (int t = s + 1; t < n; t++) {
            ...
            int maxFlow = MaxFlow(N, tempEdges, s, t);

            k = maxFlow < k ? maxFlow : k;
        }
    }

    cout << "EdgeConnectivity: " << k << endl;
